...
int AODVWaitingForDiscovery::configure(Vector<String> &conf, ErrorHandler *errh)
{
	return cp_va_kparse(conf, this, errh, "GENERATERREQ", cpkP + cpkM,
			cpElementCast, "AODVGenerateRREQ", &rreq, "NEIGHBOURS", cpkP + cpkM,
			cpElementCast, "AODVNeighbours", &neighbour_table, cpEnd);
}

void AODVWaitingForDiscovery::run_timer(Timer* t)
{
	if (_timers.find(t) != _timers.end())
	{
		AODVBuffer::Pair* pair = buffer.find_pair(_timers[t]);
		assert(pair);
		// wrong timers are horrible, this might fail if a reply is received immediately
		assert(pair->value);
		...
		else
		{
			if (pair->value->ttl < AODV_TTL_TRESHOLD)
			{
				...
				rreq->generateRREQ(_timers[t], false, pair->value->ttl);
				assert(pair->value->timer);
				pair->value->timer->schedule_after_msec(
						AODV_RING_TRAVERSAL_TIME_FACTOR
...
void AODVWaitingForDiscovery::newKnownDestination(const IPAddress & destination,
		const IPAddress & nexthop)
{
	if (AODVBuffer::Pair* pair = buffer.find_pair(destination))
	{
		pair->value->timer->unschedule();
		_timers.erase(pair->value->timer);
		delete (pair->value->timer);

		for (Vector<Packet*>::iterator iter = pair->value->packets.begin();
				iter != pair->value->packets.end();)
		{
			// RFC 6.2
			if (PAINT_ANNO(*iter) == 2)
			{ // forwarding of RREP
			...
			{ // forwarded packet
				const click_ip * ipheader = (*iter)->ip_header();
				assert(ipheader);
				neighbour_table->updateRouteLifetime(ipheader->ip_src,
						ipheader->ip_dst);
				output(0).push(*iter);
			}
			else if (PAINT_ANNO(*iter) == 2)
			{ // RREP needs changed destination
				...
				assert(false);
			// only data or RREPs allowed here
...
void AODVWaitingForDiscovery::push(int port, Packet * packet)
{
	...
	{ //DATA
		...
		if (AODVBuffer::Pair* pair = buffer.find_pair(packet->dst_ip_anno()))
		{
		...
		else
		{
			...
			int8_t hopcount = neighbour_table->getHopcount(
					packet->dst_ip_anno());
			if (hopcount != -1)
			{
			...
			waiting->timer = new Timer(this); // run handletask when timer goes off
			waiting->timer->initialize(this);
			_timers[waiting->timer] = packet->dst_ip_anno();
			waiting->timer->schedule_after_msec(
					AODV_RING_TRAVERSAL_TIME_FACTOR
	...
	{ //RREP
		...
		aodv_rrep_header * rrep = (aodv_rrep_header*) (packet->data()
				+ aodv_headeroffset);
...
void AODVWaitingForDiscovery::cleanup(CleanupStage cleanupStage)
{
	for(Timers::iterator i = _timers.begin(); i!=_timers.end(); ++i)
	{
		delete i->first;
	}
	for(AODVBuffer::iterator i = buffer.begin(); i!=buffer.end();++i)
	{
		delete i.value();
	}
}
